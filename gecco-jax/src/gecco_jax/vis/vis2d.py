...
def _plot_diffusion_trajectories(
    ...
    def pad_trajectory(traj: A['T N 2']) -> A['t n 2']:
        ...
        traj = np.pad(traj, ((1, 0), (0, 0), (0, 0), (0, 0)), constant_values=float('nan'))
...
def _plot_diffusion_sequence(
    ...
    for ax_row, sample_row, in zip(axes, sequence):
...
def _logp_figure(
    ...
    *,
    ...
    normalized_trajectories = logp_detail.trajectory_diff / diff_sigmas[None, :, None, None]
...
def _sample_figures(
    ...
    *,
    ...
    normalized_trajectory = sample_detail.trajectory_diff / diff_sigmas[None, :, None, None]
...
def make_sample_callback(
    ...
    def callback(
        ...
        sample_fig, trajectory_fig, sequence_fig = _sample_figures(model, layout, n_points, key=key)
        logger.add_figure(
            'samples',
            figure=sample_fig,
            global_step=epoch,
...
def _denoising_figure(
    ...
    data_diff, pert, denoised = [np.asarray(arr) for arr in _evaluate_denoising(model, data, key=key, n_steps=n_steps)]
    fig, axes = plt.subplots(
        n_steps,
        n_examples,
        figsize=FIG_SIZE,
    ...
    kw = dict(marker='.', s=5)
    for i, (ax_row, data_row, pert_row, denoise_row) in enumerate(zip(axes, data_diff, pert, denoised)):
        for ax, digit, data_ex, pert_ex, denoised_ex in zip(ax_row, data.ctx.digit, data_row, pert_row, denoise_row):
