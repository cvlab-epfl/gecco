...
def unshard(pytree):
    return jax.tree_map(
        lambda maybe_array: maybe_array[0]
...
class Stepper:
    ...
    def __call__(self, model, x, ctx, key, opt_state, ema_state):
        ...
        if self.ostatic is None or self.otreedef is None:
            output_tree = (jax.numpy.array(0.0), model, opt_state, ema_state)
        ...
        for val_step, data in enumerate(tqdm(dataloader, total=n_batches)):
            ...
            for metric_fn, metric_key in zip(self.metrics, keys):
                ...
                for subname, value in metric_values.items():
                    ...
                    outputs[f"{metric_fn.name}/{subname}"].append(
    ...
    def metrics_loop(
        ...
        if isinstance(self.val_dataloader, (list, tuple)):
            ...
            for subset in self.val_dataloader:
                ...
                metrics.update(
                    **{f"{subset.name}/{k}": v for k, v in submetrics.items()}
                )
            return metrics
        else:
            ...
            with pbar, logger:
                for step, data in enumerate_from(pbar, self.initial_step_number):
                    ...
                    if self.profile_path is not None:
                        step_ctx = jax.profiler.StepTraceAnnotation(
                            "train", step_num=step
                        )
                    else:
                    ...
                    if step % self.save_every == (self.save_every - 1):
                        self.save(
                            os.path.join(
                                self.save_path, CHECKPOINT_SAVE_TEMPLATE.format(step)
                            )
                        )
                        logger.add_scalar(
                            "train/mean_loss",
...
def train(
    *args, recover_from_checkpoint: bool = True, **kwargs
